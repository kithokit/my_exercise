...
class AnagramTest < MiniTest::Unit::TestCase
  ...
  def test_anagrams_when_first_is_not_match_but_the_remaining_are_angragms
    detector = Anagram.new('dog')
    anagrams = detector.match %w(dog god)
    assert_equal ['god'], anagrams
  end

  def test_anagrams_may_break2
    detector = Anagram.new('Dog')
    anagrams = detector.match %w(dog god)
    assert_equal ['god'], anagrams
  end
